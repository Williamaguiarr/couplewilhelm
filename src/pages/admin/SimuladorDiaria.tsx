...
const SimuladorDiaria: React.FC = () => {
  ...
  const resultado = useMemo(() => {
    ...
    // Guests premium: +5% per guest above 2
    const guestMult = 1 + Math.max(0, form.hospedes - 2) * 0.05;

    // Duration discount: 7+ nights -10%, 3-6 nights -5%
    const duracaoMult =
      form.duracaoMinima >= 7 ? 0.9 : form.duracaoMinima >= 3 ? 0.95 : 1;

    // Rating bonus: above 4.5 → +10%, above 4.0 → +5%
    const ratingMult =
    ...
    // Estimated monthly revenue (assuming 70% occupancy)
