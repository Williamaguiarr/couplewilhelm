-- Atualiza a função calculate_reserva_liquido_proprietario
...
BEGIN
    ...
    -- Regra Correta: Valor Líquido = Valor Bruto - Taxa de Limpeza - Comissão OTA
    v_valor_liquido := COALESCE(p_valor_bruto, 0) - COALESCE(p_taxa_limpeza, 0) - COALESCE(p_comissao_plataforma, 0);
    
    -- Comissão ADM = Valor Líquido × Percentual ADM (Clamped to 0 if base is negative)
    v_comissao_adm := GREATEST(0, v_valor_liquido) * COALESCE(v_taxa_comissao, 0.25);
    
    -- Repasse ao Proprietário = Valor Líquido - Comissão ADM
    RETURN v_valor_liquido - v_comissao_adm;
END;
$function$;

-- Atualiza a função process_reserva_financeiro
...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') AND NOT v_financial_changed AND (OLD.auditada = FALSE AND NEW.auditada = TRUE) THEN
        ...
        -- Garante que valor_base_comissao (Valor Líquido) esteja preenchido se possível
    ...
    -- 6. Recálculo Financeiro (Nova Regra - Valor Líquido Deduzido de Limpeza e OTA)
    -- Valor Líquido = Valor Bruto - Taxa de Limpeza - Comissão OTA
    NEW.valor_base_comissao := COALESCE(NEW.valor_bruto, 0) - COALESCE(NEW.taxa_limpeza, 0) - COALESCE(NEW.comissao_plataforma, 0);
         
    -- Comissão ADM = Valor Líquido * Taxa do Snapshot (Clamped to 0 if base is negative)
    NEW.valor_comissao_admin := GREATEST(0, NEW.valor_base_comissao) * (COALESCE(v_taxa_comissao, 25) / 100.0);
         
    -- Repasse ao Proprietário = Valor Líquido - Comissão ADM
    NEW.valor_liquido_proprietario := NEW.valor_base_comissao - NEW.valor_comissao_admin;
